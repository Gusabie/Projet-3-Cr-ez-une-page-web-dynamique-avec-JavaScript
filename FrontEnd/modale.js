...
function generergallerymodal(works) { //appel des informations de works
...
//Supprimer un element
    ...
    deleteimg.addEventListener("click", async (event) => {// agit au clique
    ...
    const id = projet.dataset.id; // cible l'id de l'element parent de l'icone
        ...
        try {
          ...
          if (response.ok) { 
            ...
            const Updategallery = await fetch(UrlAPIworks).then((response) => response.json())//nouvelle requette pour mettre la gallerie à jour
            ...
            console.error(`Échec de la suppression de l'élément avec l'ID ${id}.`);//affiche le resultat dans la console si la requete echoue
          }
...
function afficherformulaire() {
    ...
    if (modalgallery.classList.contains("active")) { //si la modale gallerie est active
...
function Retourmodal() { 
  ...
  if (modalform.classList.contains("active")) { //si la modale formulaire est active
...
for (let i = 0; i < modalTriggers.length; i++) { //pour chaque element ayant la classe "modalTriggers"
...
function toggleModal(){
    ...
    if (!galmodal.classList.contains("active")){ //si galmodal n'est pas active
    galmodal.classList.toggle("active"); //l'activer
    } else { //sinon
      formmodal.classList.remove("active"); //desactiver la modale formulaire
    }
    if (!modalContainer.classList.contains("active")){ //si le conteneur des modales n'est pas actif
      formmodal.classList.remove("active"); //desactiver la modale formulaire
...
valider.addEventListener("click", async function(e){//ecoute d'evenement au click sur le bouton "valider"
        e.preventDefault();      
  //Recupere les valeurs du formulaire
        ...
        if (response.ok) {
          ...
          const Updategallerymodal = await fetch(UrlAPIworks).then((response) => response.json())//nouvelle requette pour mettre la gallerie à jour
